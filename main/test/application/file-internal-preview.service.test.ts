...
describe('Test file-internal-preview.service',()=>{
    beforeEach(()=>{
    ...
    it('Test con documento non presente',()=>{
        ...
        const preview = container.resolve(FileInternalPreviewService);
        expect(preview.execute('uuid','uuid')).rejects.toThrow(`Non esiste un documento con questo UUID: uuid`);
    });
    ...
    it('Test con allegato non trovato',()=>{
        ...
        const preview = container.resolve(FileInternalPreviewService);
        expect(preview.execute('uuid','uuid')).rejects.toThrow(`Non esiste un file con UUID: uuid associato al documento con UUID: uuid`);
    });
