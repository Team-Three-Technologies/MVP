...
import { DipDashboardContainer } from '../app/dip-dashboard-container/dip-dashboard-container';
import { ElectronIpc } from '../app/services/electron-ipc';
import { IPC_CHANNELS } from '@shared/ipc-channels';

describe('DipDashboard', () => {
  ...
  it('mostra i dettagli del documento selezionato nel template', async () => {
    ...
    await component.onDocumentSelected('doc-321');
    fixture.detectChanges();

    const text = fixture.nativeElement.textContent as string;
    expect(text).toContain('Dettagli Documento');
    expect(text).toContain('Documento dettagliato');
    expect(text).toContain('REG-10');
  });
});
